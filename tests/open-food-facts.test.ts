...
import { preparaArchivio, prodottiSample, prodottoProva, } from "./supporto-archivio";
...
async function preparaHttp(contesto: TestContext, rispondi: (richiesta: IncomingMessage, risposta: ServerResponse) => void) {
...
const stato = (atteso: number) => (errore: unknown) => errore instanceof ErroreApplicazione && errore.stato === atteso;
test("cache presente e alias GTIN non eseguono richieste HTTP", async (contesto) => {
    const { archivio } = (await preparaArchivio(contesto, true, {
        richiediOff: async () => {
    ...
    assert.equal((await archivio.recuperaProdotto("013800810168")).code, "0013800810168");
});
test("cache assente recupera venti prodotti tramite HTTP con User-Agent e conserva nutrienti ingredienti e allergeni", async (contesto) => {
    ...
    const indirizzoOff = await preparaHttp(contesto, (richiesta, risposta) => {
        richieste++;
        assert.ok(new URL(richiesta.url!, "http://localhost").searchParams.get("user_agent")?.startsWith("FridgeBrain/1.0"));
    ...
    for (const atteso of prodottiSample) {
        ...
        const riga = (await archivio.database.cache_prodotti_off.get(atteso.code))!;
...
test("scansioni simultanee equivalenti condividono il recupero HTTP", async (contesto) => {
    let chiamate = 0;
    const originale = prodottiSample.find((voce) => voce.code === "0013800810168")!;
    const { archivio } = (await preparaArchivio(contesto, false, {
        richiediOff: async () => {
            chiamate++;
...
for (const [descrizione, risposta, atteso] of [
    ...
    test(`${descrizione}: errore comprensibile e nessuna cache contaminata`, async (contesto) => {
        ...
        await assert.rejects(archivio.recuperaProdotto(prodottoProva.code), stato(atteso));
        assert.equal(await archivio.database.cache_prodotti_off.count(), 0);
    });
}
test("errore di rete non impedisce l'utilizzo di prodotti già salvati", async (contesto) => {
    ...
    await assert.rejects(archivio.recuperaProdotto(prodottoProva.code), stato(503));
    assert.deepEqual(await archivio.recuperaProdotto(prodottiSample[0].code), prodottiSample[0]);
});
test("timeout interrompe anche un corpo HTTP incompleto", async (contesto) => {
    ...
    await assert.rejects(archivio.recuperaProdotto(prodottoProva.code), stato(504));
});
test("barcode non valido non contatta OFF e il limite 429 introduce una pausa senza bloccare la cache", async (contesto) => {
    ...
    await assert.rejects(archivio.recuperaProdotto(prodottoProva.code), stato(429));
...
test("aggiornamento cache conserva il primo recupero e non modifica lo snapshot dell'inventario", async (contesto) => {
    ...
    await archivio.database.cache_prodotti_off.update(originale.code, { recuperato_il: "2020-01-01", aggiornato_il: "2020-01-01" });
    (await archivio.salvaCacheOff({ ...originale, product_name: "Nome aggiornato" }));
    const riga = (await archivio.database.cache_prodotti_off.get(originale.code))!;
    assert.equal(riga.recuperato_il, "2020-01-01");
    assert.notEqual(riga.aggiornato_il, "2020-01-01");
    assert.equal((await archivio.prodotto(originale.code)).product_name, "Nome aggiornato");
    assert.deepEqual((await archivio.voce(voce.id)).prodotto, originale);
});
test("cache e lista della spesa persistono dopo la riapertura di IndexedDB", async (contesto) => {
    const { archivio } = await preparaArchivio(contesto, false);
    await archivio.aggiungiSpesa({ nome: "Latte" });
    await archivio.salvaCacheOff(prodottoProva);
    const nomeDatabase = archivio.database.name;
    archivio.chiudi();
    const riaperto = new ArchivioFridgeBrain({ nomeDatabase });
    try {
        assert.equal((await riaperto.spesa())[0].nome, "Latte");
        assert.deepEqual(await riaperto.prodotto(prodottoProva.code), prodottoProva);
    }
    finally {
        riaperto.chiudi();
    }
